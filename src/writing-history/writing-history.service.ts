...
export class WritingHistoryService {
  ...
  constructor(
    @InjectRepository(WritingHistory) private writingHistoryRepository: Repository<WritingHistory>,
  ) { }
  ...
  async findAll(currentPage: number, limit: number, qs: string) {
    ...
    const [items, totalItems] = await this.writingHistoryRepository.findAndCount({
      where: filter as any,
      skip,
      take,
      order,
